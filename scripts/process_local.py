print("[DEBUG] Script started: process_local.py")

import os
import asyncio
from dotenv import load_dotenv

load_dotenv(os.path.join(os.path.dirname(__file__), "../.env"))

from app.pipeline import process_file_local
from app.llm_client import OpenAILLMClient
from app.transcribe import WhisperTranscriber


async def main():
    print("[DEBUG] Starting main() in process_local.py")

    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    path = os.path.join(project_root, "lecture.mp4")
    print(f"[DEBUG] Checking for lecture.mp4: Exists={os.path.exists(path)} | Path={path}")

    # ✅ Extract entire audio (not just 10s)
    import ffmpeg
    full_audio_path = os.path.join(project_root, "lecture_full.wav")
    try:
        (
            ffmpeg.input(path)
            .output(full_audio_path, ar=16000, ac=1)
            .overwrite_output()
            .run()
        )
        print(f"[DEBUG] Created full audio: {full_audio_path}")
    except Exception as e:
        print(f"[ERROR] ffmpeg failed: {e}")
        return

    llm = OpenAILLMClient()
    trans = WhisperTranscriber()
    print("[DEBUG] Starting process_file_local...")
    try:
        job = await process_file_local(full_audio_path, "lecture_full.wav", llm, trans, ["hi-IN"])
        print(f"[DEBUG] Job result: {job}")
    except Exception as e:
        print(f"[ERROR] process_file_local failed: {e}")


if __name__ == "__main__":
    asyncio.run(main())
